...
# Generic timing stage for research/unit_test_variations/<name>/config.yaml.
# Launch through sub.sh, which sets CONFIG and REPO.
echo "===================================="
echo "SLURM JOB STARTED  (timing)"
...
start_time=$(date +%s)
python "$REPO/research/shear_bias/run.py" --task timing -c "$CONFIG"
end_time=$(date +%s)
