...
from helpers_ngmix_mcal_with_sn import (
    _get_priors,
    progress,
    make_struct,
    process_obs,
...
def parse_args():
    ...
    parser.add_argument(
        "-c", "--config",
        default="config_with_sn.yaml",
        help="Path to YAML configuration file (default: config_with_sn.yaml)",
    )
...
def process_single_object(args):
    ...
    data_p, mcal_images_p, raw_image_p, psf_image = process_obs(obsp, boot)
    data_m, mcal_images_m, raw_image_m, _         = process_obs(obsm, boot)
    return data_p, data_m, mcal_images_p, mcal_images_m, raw_image_p, raw_image_m, psf_image


args = [(i, SEED, NOISE, SHEAR_TRUE) for i in range(NOBS)]
...
def _run_shearnet_batch(img_buffer, data_list_p, data_list_m):
    if state is None:
         return
    n = len(img_buffer)
    base = len(data_list_p) - n
    stypes = list(img_buffer[0][2].keys())

    # per-stype inference (fills g_sn in each struct row)
    for stype in stypes:
        gal_p  = jnp.stack([r[2][stype][0] for r in img_buffer])
        psf_p  = jnp.stack([r[2][stype][1] for r in img_buffer])
        gal_m  = jnp.stack([r[3][stype][0] for r in img_buffer])
        psf_m  = jnp.stack([r[3][stype][1] for r in img_buffer])
    ...
    # raw inference (g_sn_raw)
    raw_p  = jnp.stack([r[4] for r in img_buffer])
    raw_m  = jnp.stack([r[5] for r in img_buffer])
    psf_raw = jnp.stack([r[6] for r in img_buffer])
...
if state is not None:
    tab_p["g_sn_raw"] = np.concatenate(g_sn_raw_list_p)
    tab_m["g_sn_raw"] = np.concatenate(g_sn_raw_list_m)
else:
    tab_p["g_sn_raw"] = np.full((len(tab_p), 2), np.nan)
    tab_m["g_sn_raw"] = np.full((len(tab_m), 2), np.nan)
...
if state is not None:
    (
        ...
        _,
        _,
    ) = jackknife_mc_v2(tab_p, tab_m, shear_true, njac=NJAC, g_col="g_sn_raw", r11_col="r11_sn")
