def main():
    ...
    print("Starting deconvolution training...")
    
    # For now, let's use simple defaults
    ...
    try:
        combined_images, labels = generate_dataset(
            args.samples, psf_sigma=0.25, exp='ideal', seed=args.seed, 
            npix=53, scale=0.141, nse_sd=1e-5, neural_metacal=True
        )
        ...
        psf_combined_images, _ = generate_dataset(
            args.samples, psf_sigma=0.25, exp='ideal', seed=args.seed,
            npix=53, scale=0.141, nse_sd=1e-5, process_psf=True
        )
        _, psf_images = split_combined_images(psf_combined_images, has_psf=True, has_clean=False)
        
    except Exception as e:
        print(f"Error during dataset generation: {e}")
        print("This might be due to GalSim numerical instabilities.")
        print("Try reducing the number of samples or adjusting galaxy parameters.")
        return
    ...
    # Debug: Check for any problematic images
    print("Checking for problematic images...")
    galaxy_min, galaxy_max = galaxy_images.min(), galaxy_images.max()
    psf_min, psf_max = psf_images.min(), psf_images.max()
    target_min, target_max = clean_images.min(), clean_images.max()
    
    print(f"Galaxy range: [{galaxy_min:.3e}, {galaxy_max:.3e}]")
    print(f"PSF range: [{psf_min:.3e}, {psf_max:.3e}]")
    print(f"Target range: [{target_min:.3e}, {target_max:.3e}]")
    
    # Check for any zero images (from GalSim failures)
    zero_galaxies = jnp.sum(jnp.sum(galaxy_images, axis=(1,2)) == 0)
    zero_psfs = jnp.sum(jnp.sum(psf_images, axis=(1,2)) == 0) 
    zero_targets = jnp.sum(jnp.sum(clean_images, axis=(1,2)) == 0)
    
    print(f"Zero images - Galaxy: {zero_galaxies}, PSF: {zero_psfs}, Target: {zero_targets}")
    
    if zero_galaxies > 0 or zero_psfs > 0 or zero_targets > 0:
        print("Warning: Some images are all zeros due to GalSim failures.")
        print("Consider reducing galaxy size parameters or using different seeds.")

    rng_key = random.PRNGKey(args.seed)
    
    # Check sample shapes before model initialization
    print(f"Sample galaxy shape: {galaxy_images[0].shape}")
    print(f"Sample PSF shape: {psf_images[0].shape}")
    print(f"Sample target shape: {clean_images[0].shape}")
    ...
    try:
        ...
        print("This might be due to malformed images from GalSim failures.")
        return"""Command-line interface for training PSF deconvolution models."""
