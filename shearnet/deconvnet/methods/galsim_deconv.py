"""
FFT-based GalSim PSF deconvolution methods for benchmarking against what ngmix uses
"""
...
def deconvolve_single(obs):
    ...
    try:
        ...
        print(f"GalSim deconvolution failed for single galaxy: {e}")
        return np.zeros_like(obs.image)


def deconvolve(observations: List[ngmix.Observation], 
                      n_jobs: int = None, seed: int = 42) -> jnp.ndarray:
    """
    Perform PSF deconvolution using GalSim approach
    ...
    if n_jobs == 1:
        ...
        for obs in observations:
            result = deconvolve_single(obs)
            deconvolved_images.append(result)
    else:
        # Multiprocessing
        with ProcessPoolExecutor(max_workers=n_jobs) as executor:
            futures = {executor.submit(deconvolve_single, obs): i 
