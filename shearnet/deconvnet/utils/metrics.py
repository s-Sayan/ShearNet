...
def normalize_data(galaxy_images: jnp.ndarray, psf_images: jnp.ndarray, 
                     target_images: jnp.ndarray,) -> Tuple[Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray], Tuple[jnp.ndarray, jnp.ndarray]]:
    """
    Z-score normalize three arrays independently.

    Returns:
        normalized (galaxy_images, psf_images, target_images)
        params (means, stds) where each is shape (3,)
    """
    arrays = [galaxy_images, psf_images, target_images]
    
    means = jnp.array([jnp.mean(a) for a in arrays])
    stds = jnp.array([jnp.std(a) for a in arrays])

    normalized = tuple((a - m) / s for a, m, s in zip(arrays, means, stds))
    
    return normalized, (means, stds)

def inverse_normalized_data(galaxy_images: jnp.ndarray, psf_images: jnp.ndarray, target_images: jnp.ndarray, means: jnp.ndarray, stds: jnp.ndarray
) -> Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray]:
    """
    Invert z-score normalization using stored means and stds.
    """
    normed = [galaxy_images, psf_images, target_images]
    original = tuple((n * s) + m for n, m, s in zip(normed, means, stds))
    return original
...
def eval_galsim_deconv(observations: List, target_images: jnp.ndarray, **kwargs) -> Dict[str, Any]:
    """
    Evaluate GalSim-based deconvolution method using pre-existing observations.
    ...
    # Generate galsim deconvolution predictions using pre-existing observations
    predictions = deconvolve(observations, **kwargs_copy)
    ...
    # Print results
    print(f"\n{BOLD}=== GalSim Deconvolution (Using Pre-existing Obs) ==={END}")
    print(f"Evaluation Time: {BOLD}{CYAN}{total_time:.2f} seconds{END}")
    print(f"Mean Squared Error (MSE): {BOLD}{YELLOW}{mse:.6e}{END}")
    print(f"Mean Absolute Error (MAE): {BOLD}{YELLOW}{mae:.6e}{END}")
    print(f"Peak Signal-to-Noise Ratio (PSNR): {BOLD}{CYAN}{psnr:.2f} dB{END}")
...
def compare_methods(neural_results: Dict[str, Any], galsim_results: Dict[str, Any]) -> None:
    """
    Compare results from neural network and GalSim deconvolution methods.
    
    Args:
        neural_results: Results from neural network deconvolution
        galsim_results: Results from galsim deconvolution
    """
    ...
    for metric in metrics:
        if metric in neural_results and metric in galsim_results:
            neural_val = neural_results[metric]
            galsim_val = galsim_results[metric]
            
            # Determine winner (lower is better for most metrics, except PSNR and SSIM)
            if metric in higher_better:
                winner = "Neural" if neural_val > galsim_val else "GalSim"
            ...
            if metric == 'time_taken':
                ...
                galsim_str = f"{galsim_val:.2f}s"
            elif metric in ['psnr']:
                neural_str = f"{neural_val:.2f}"
                galsim_str = f"{galsim_val:.2f}"
            elif metric in ['ssim']:
                neural_str = f"{neural_val:.4f}"
                galsim_str = f"{galsim_val:.4f}"
            else:
                neural_str = f"{neural_val:.3e}"
                galsim_str = f"{galsim_val:.3e}"
            
            print(f"{metric.upper():<25} {neural_str:<15} {galsim_str:<15} {winner_color}{winner}{END}")
    ...
    for metric in ['mse', 'mae', 'normalized_mse']:  # Lower is better
        if neural_results[metric] < galsim_results[metric]:
            neural_wins += 1
        else:
            galsim_wins += 1
    ...
    if neural_wins > galsim_wins:
        print(f"{BOLD}{GREEN}Overall Winner: Neural Network Deconvolution{END}")
        print(f"Neural network wins {neural_wins} out of {neural_wins + galsim_wins} key metrics")
    elif galsim_wins > neural_wins:
        print(f"{BOLD}{RED}Overall Winner: GalSim Deconvolution{END}")
        print(f"GalSim method wins {galsim_wins} out of {neural_wins + galsim_wins} key metrics")
    else:
        print(f"{BOLD}{YELLOW}Result: Tie between methods{END}")
        print(f"Each method wins {neural_wins} out of {neural_wins + galsim_wins} key metrics")
