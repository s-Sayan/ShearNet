...
# ANSI color codes for pretty printing (empty when output is not a terminal).
BOLD = ansi("\033[1m")
YELLOW = ansi("\033[93m")
CYAN = ansi("\033[96m")
GREEN = ansi("\033[92m")
...
def remove_nan_preds_multi(
    pred1: np.ndarray, pred2: np.ndarray, labels: np.ndarray
...
def eval_ngmix(
    test_obs, test_labels, seed=1234, psf_model="gauss", gal_model="gauss"
) -> Dict[str, Any]:
    ...
    valid_mask = (
        np.isfinite(r11_array)
        & np.isfinite(r22_array)
        & np.isfinite(r12_array)
        & np.isfinite(r21_array)
    )
    R = np.array(
        [
    ...
    R_per_gal = np.stack(
        [np.stack([r11_array, r12_array], axis=1), np.stack([r21_array, r22_array], axis=1)], axis=1
    )
...
def _per_label_metrics(preds, labels, output_keys):
    ...
    if "g1" in _ok and "g2" in _ok:
        ...
        loss_per_label["g1g2_combined"] = float(
            optax.l2_loss(preds[:, g_idx], labels[:, g_idx]).mean()
        )
...
def eval_model(
    state, test_images, test_labels, output_keys=("g1", "g2"), batch_size=32
) -> Dict[str, Any]:
    ...
    total_samples = 0
    total_bias = 0
    _label_keys = list(output_keys) + (
        ["g1g2_combined"] if "g1" in output_keys and "g2" in output_keys else []
    )
    ...
    avg_loss_per_label = {
        label: total / total_samples for label, total in total_loss_per_label.items()
    }
    avg_bias_per_label = {
        label: total / total_samples for label, total in total_bias_per_label.items()
    }
    total_time = time.time() - start_time
...
def fork_eval_model(
    state, test_images, test_psf_images, test_labels, output_keys=("g1", "g2"), batch_size=32
) -> Dict[str, Any]:
...
def calculate_response_matrix(
    state, observations, batch_size=32, h=0.01, model_type="standard", psf_images=None
):
    ...
    R_per_galaxy = jnp.stack(
        [jnp.stack([R_11, R_12], axis=1), jnp.stack([R_21, R_22], axis=1)], axis=1
    )
...
def calculate_multiplicative_bias(
    state,
    obs_g1_pos,
    obs_g1_neg,
    obs_g2_pos,
...
def calculate_multiplicative_bias_ngmix(
    obs_g1_pos,
    obs_g1_neg,
    obs_g2_pos,
    obs_g2_neg,
    ...
    """Compute NGmix shear bias (counterpart of :func:`calculate_multiplicative_bias`).
