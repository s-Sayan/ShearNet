...
def extract_psf_properties_from_obs(observations):
    ...
    for obs in observations:
        try:
            ...
            logger.info(f"Warning: Could not extract PSF properties: {e}")
            psf_e1_list.append(0.0)
            psf_e2_list.append(0.0)
            psf_T_list.append(np.nan)

    return {"e1": np.array(psf_e1_list), "e2": np.array(psf_e2_list), "T": np.array(psf_T_list)}
...
def calculate_image_moments(image):
    ...
    return {"e1": float(e1), "e2": float(e2), "T": float(T), "flux": float(total_flux)}


def plot_psf_systematics(
    predicted_shears,
    psf_properties,
    ...
    logger.info("\n" + "=" * 70)
    logger.info("PSF SYSTEMATICS - LINEAR FIT RESULTS")
    logger.info("=" * 70)
    ...
    for dataset in datasets:
        ...
        if psf_range < 1e-10:
            logger.info(
                f"Warning: PSF e1 has insufficient variation (range={psf_range:.2e}). "
                f"Skipping for {name}."
            )
            ax_e1.scatter(
                psf_e1.mean(),
        ...
        mean_g1, _, _ = binned_statistic(psf_e1, shears[:, 0], statistic="mean", bins=bins)
        std_g1, _, _ = binned_statistic(psf_e1, shears[:, 0], statistic="std", bins=bins)
        count_g1, _, _ = binned_statistic(psf_e1, shears[:, 0], statistic="count", bins=bins)
        ...
        logger.info(f"{name} - PSF e1 bins: galaxy counts = {count_g1_valid}")
        logger.info(f"{name} - PSF e1 bins: error bars = {stderr_g1_valid}")

        # Plot binned data points with error bars
        ax_e1.errorbar(
            bin_centers_valid,
            mean_g1_valid,
            yerr=stderr_g1_valid,
            fmt=marker,
            color=color,
            label=name,
            markersize=6,
        ...
        try:
            ...
            logger.info(
                f"  Linear fit: g1 = ({slope:.6e} ± {slope_err:.6e}) * PSF_e1 "
                f"+ ({intercept:.6e} ± {intercept_err:.6e})"
            )
            logger.info(f"  α₁ (leakage coefficient) = {slope:.6e} ± {slope_err:.6e}")
            ...
            logger.info(f"Warning: Could not fit PSF e1 for {name}: {e}")
            pass
    ...
    for dataset in datasets:
        ...
        if psf_range < 1e-10:
            logger.info(
                f"Warning: PSF e2 has insufficient variation (range={psf_range:.2e}). "
                f"Skipping for {name}."
            )
            ax_e2.scatter(
                psf_e2.mean(),
        ...
        mean_g2, _, _ = binned_statistic(psf_e2, shears[:, 1], statistic="mean", bins=bins)
        std_g2, _, _ = binned_statistic(psf_e2, shears[:, 1], statistic="std", bins=bins)
        count_g2, _, _ = binned_statistic(psf_e2, shears[:, 1], statistic="count", bins=bins)
        ...
        # Verification
        logger.info(f"{name} - PSF e2 bins: galaxy counts = {count_g2_valid}")
        logger.info(f"{name} - PSF e2 bins: error bars = {stderr_g2_valid}")

        # Plot binned points
        ax_e2.errorbar(
            bin_centers_valid,
            mean_g2_valid,
            yerr=stderr_g2_valid,
            fmt=marker,
            color=color,
            label=name,
            markersize=6,
        ...
        try:
            ...
            logger.info(
                f"  Linear fit: g2 = ({slope:.6e} ± {slope_err:.6e}) * PSF_e2 "
                f"+ ({intercept:.6e} ± {intercept_err:.6e})"
            )
            logger.info(f"  α₂ (leakage coefficient) = {slope:.6e} ± {slope_err:.6e}")
            ...
            logger.info(f"Warning: Could not fit PSF e2 for {name}: {e}")
            pass
    ...
    for dataset in datasets:
        ...
        mean_g1, _, _ = binned_statistic(psf_T, shears[:, 0], statistic="mean", bins=bins)
        std_g1, _, _ = binned_statistic(psf_T, shears[:, 0], statistic="std", bins=bins)
        count_g1, _, _ = binned_statistic(psf_T, shears[:, 0], statistic="count", bins=bins)
        ...
        try:
            ...
            logger.info(
                f"  Linear fit: g1 = ({slope:.6e} ± {slope_err:.6e}) * T_PSF "
                f"+ ({intercept:.6e} ± {intercept_err:.6e})"
            )
            logger.info(f"  β₁ (size leakage coefficient) = {slope:.6e} ± {slope_err:.6e}")
            ...
            logger.info(f"Warning: Could not fit PSF T (g1) for {name}: {e}")
    ...
    for dataset in datasets:
        ...
        mean_g2, _, _ = binned_statistic(psf_T, shears[:, 1], statistic="mean", bins=bins)
        std_g2, _, _ = binned_statistic(psf_T, shears[:, 1], statistic="std", bins=bins)
        count_g2, _, _ = binned_statistic(psf_T, shears[:, 1], statistic="count", bins=bins)
        ...
        try:
            ...
            logger.info(
                f"  Linear fit: g2 = ({slope:.6e} ± {slope_err:.6e}) * T_PSF "
                f"+ ({intercept:.6e} ± {intercept_err:.6e})"
            )
            logger.info(f"  β₂ (size leakage coefficient) = {slope:.6e} ± {slope_err:.6e}")
            ...
            logger.info(f"Warning: Could not fit PSF T (g2) for {name}: {e}")
    ...
    if path:
        ...
        logger.info(f"PSF systematics plot saved to: {path}")

    return fig, results


def plot_psf_systematics_from_eval(
    test_obs,
    predicted_labels,
    ...
    if len(predicted_labels) != len(test_obs):
        logger.info("Warning: Length mismatch detected!")
        logger.info(f"  predicted_labels: {len(predicted_labels)}")
        logger.info(f"  test_obs: {len(test_obs)}")
        logger.info("  Trimming test_obs to match predicted_labels length...")
        # Assume predicted_labels was filtered and test_obs needs to match
        test_obs = test_obs[: len(predicted_labels)]

    # Extract PSF properties from observations (defined in this module).
    logger.info("Extracting PSF properties from observations...")
    psf_properties = extract_psf_properties_from_obs(test_obs)

    # Check if we have valid PSF properties
    valid_psf = ~(
        np.isnan(psf_properties["T"])
        | np.isnan(psf_properties["e1"])
        | np.isnan(psf_properties["e2"])
    )

    num_valid = np.sum(valid_psf)
    logger.info(f"Valid PSF measurements: {num_valid}/{len(test_obs)}")

    if num_valid < 10:
        logger.info(
            f"Warning: Only {num_valid} valid PSF measurements. Skipping PSF systematics plot."
        )
        return None, None
    ...
    logger.info("PSF property ranges:")
    logger.info(f"  e1: {e1_range:.6f}")
    logger.info(f"  e2: {e2_range:.6f}")
    logger.info(f"  T:  {T_range:.6f}")

    if e1_range < 1e-10 and e2_range < 1e-10 and T_range < 1e-10:
        logger.info("Warning: PSF properties have no variation. Cannot analyze systematics.")
        return None, None
    ...
    if ngmix_preds is not None:
        ...
        if len(ngmix_preds_np) != len(test_obs):
            logger.info("Warning: NGmix predictions length mismatch!")
            logger.info(f"  ngmix_preds: {len(ngmix_preds_np)}")
            logger.info(f"  test_obs: {len(test_obs)}")
            # Try to align - if ngmix was filtered, trim test_obs accordingly
            if len(ngmix_preds_np) < len(test_obs):
                # This shouldn't happen if we already trimmed test_obs above
                logger.info(f"  Using first {len(ngmix_preds_np)} observations for NGmix")
        ...
        ngmix_valid_mask = ~(
            np.isnan(ngmix_shears_valid[:, 0]) | np.isnan(ngmix_shears_valid[:, 1])
        )

        if np.sum(ngmix_valid_mask) > 10:
            logger.info(
                f"NGmix valid predictions: {np.sum(ngmix_valid_mask)}/{len(ngmix_shears_valid)}"
            )
            ...
            logger.info(
                f"Warning: Only {np.sum(ngmix_valid_mask)} valid NGmix predictions. "
                f"Not plotting NGmix."
            )
...
def calculate_psf_leakage_coefficients(predicted_shears, psf_properties, true_shears=None):
    ...
    try:
        popt1, pcov1 = curve_fit(linear_func, psf_properties["e1"], residuals[:, 0])
        results["alpha_1"] = {
            "value": popt1[0],
    ...
    try:
        popt2, pcov2 = curve_fit(linear_func, psf_properties["e2"], residuals[:, 1])
        results["alpha_2"] = {
            "value": popt2[0],
    ...
    try:
        popt3, pcov3 = curve_fit(linear_func, psf_properties["T"], residuals[:, 0])
        results["beta_1"] = {
            "value": popt3[0],
    ...
    try:
        popt4, pcov4 = curve_fit(linear_func, psf_properties["T"], residuals[:, 1])
        results["beta_2"] = {
            "value": popt4[0],
