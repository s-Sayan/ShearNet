...
def plot_residuals(target_images, neural_preds, galsim_deconv_preds, num_samples=5, save_path=None):
    ...
    for i in range(num_samples):
        ...
        # Galsim.deconv residual
        galsim_deconv_residual = galsim_deconv_preds[i] - target_images[i]
        vmax = np.max(np.abs(galsim_deconv_residual))
        axes[2, i].imshow(galsim_deconv_residual, cmap='RdBu_r', vmin=-vmax, vmax=vmax)
        axes[2, i].set_title(f'Galsim.deconv Residual {i+1}')
        axes[2, i].axis('off')
...
def plot_deconv_spatial_residuals(target_images: jnp.ndarray, neural_predictions: jnp.ndarray,
