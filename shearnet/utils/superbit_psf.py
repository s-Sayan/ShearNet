"""PSF model wrapper copied verbatim from the superbit-lensing repository.

Previously imported as
``superbit_lensing.medsmaker.superbit.psf_extender``. It is vendored here
(unchanged apart from the internal utility import being redirected to the
sibling :mod:`shearnet.utils.superbit` module) so that the ``research`` code
can build PSFEx / PIFF shape maps without installing ``superbit-lensing``.

Source: https://github.com/superbit-collaboration/superbit-lensing
        (``superbit_lensing/medsmaker/superbit/psf_extender.py``)

Nothing in this module is original to ShearNet; it is a vendored copy. Any
