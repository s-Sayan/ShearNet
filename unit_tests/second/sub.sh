...
# Submit timing job, after training completes
JOBID_TIMING=$(sbatch --dependency=afterok:$JOBID_TRAIN ./timing/run_timing.sh | awk '{print $4}')
echo "Submitted leakage job: $JOBID_TIMING (depends on $JOBID_TRAIN)"
